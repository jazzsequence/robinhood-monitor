...
CLAUDE_SYSTEM_PROMPT = (
    ...
    "TRIM SIGNALS — treat any of these as a reason to consider a partial trim even without a "
    "broken thesis: (a) position is 30%+ above its MA50, (b) single position exceeds 35% of "
    "portfolio, (c) a high-momentum opportunity exists that the portfolio doesn't yet capture. "
    ...
    "(1) any positions to EXIT or TRIM — check trim signals above before writing 'none'; "
    "(2) your total buy capacity (cash + trim proceeds) and any positions to ADD TO or new "
    "positions to ENTER within that limit — including small fractional purchases; "
    "(3) one key thing to watch today — this must be something not already actionable; if it "
    "is actionable today, put it in sections (1) or (2) instead of deferring it here. "
