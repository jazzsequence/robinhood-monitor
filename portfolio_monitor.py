...
CLAUDE_SYSTEM_PROMPT = (
    ...
    "PART 2 — Full analysis in three blocks:\n\n"
    "TRIMS/EXITS: Check trim signals for all positions. Write one bold action line per trim "
    "using **markdown bold** (e.g. **TRIM ARM — Sell $50**) followed by one sentence of reasoning. "
    "Skip positions that require no action. If nothing to trim, say so in one sentence.\n\n"
    "BUYS: State total capital available (cash + trim proceeds) in one line. Then one bold action "
    "line per buy (e.g. **BUY MRVL — $35**) with one sentence of reasoning each. "
    "State each buy exactly once — no capital math summary.\n\n"
    "HOLDS: Group remaining positions in 1-2 sentences with brief descriptions of each. "
    "Do not write individual paragraphs per position.\n\n"
    "End with a single line starting with **WATCH:** describing the one thing to monitor that "
    "is not yet actionable today. If something is actionable, put it in TRIMS or BUYS instead.\n\n"
    ...
    "(a) the opportunity IS compelling — identify a partial trim of a winner to fund it and recommend "
    "both moves together, or (b) the opportunity is NOT compelling enough to justify disrupting a "
